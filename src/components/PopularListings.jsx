...
const PopularListings = () => {
    ...
    const filteredAppareals = data.filter(
        (cloth) => cloth.status === "Popular"
    )
    ...
    <main>
        <section className='w-11/12 container mx-auto py-16'>
            <div className='flex justify-between items-center mb-12'>
                <h3 className='font-medium text-3xl'>MOST POPULAR</h3>
            </div>

            <div className='grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5 lg:gap-8'>
                {filteredAppareals.slice(0, 3).map((apparel)=>{
                    return (
                    <div key={apparel.id} className="relative group overflow-hidden">
                        ...
                        <div className='mt-2'>
                            ...
                            <p>₦{apparel.price.toLocaleString()}</p>
                        </div>
