...
const All = () => {
    ...
    const allProducts = product.filter(
        (cloth) => (activeTab === "All" || cloth.status === activeTab)
    )
    ...
    <main>
        <section className='w-11/12 container mx-auto py-16'>
            <div className='md:flex justify-between mb-12 py-6 '>
              <h3 className='font-medium text-[32px] mb-6'>All Products</h3>
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-5 lg:gap-7'>
                ...
                {/* CLICKABLE CARD */}
                    <Link to={`/collections/${item.id}`} className='block'>
                        <div className='relative w-full aspect-3/4 sm:aspect-4/5 lg:aspect-3/4'>
                            ...
                            <img src={item?.images?.[1] || item?.images?.[0]} alt={item.name} className="absolute inset-0 w-full h-full object-cover opacity-0 transition-opacity duration-500  group-hover:opacity-100"/>
    
                            <div className='absolute top-2 left-2 sm:top-3 sm:left-3 bg-black/80 text-white px-2 py-1 rounded-full text-[10px] sm:text-xs'>{item.status}</div>
                        </div>
                    </Link>
